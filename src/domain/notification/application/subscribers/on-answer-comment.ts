...
export class OnAnswerComment implements EventHandler {
    ...
    private async sendAnswerCommentNotification({ answerId }: AnswerCommentEvent) {

        const answer = await this.answersRepository.findById(answerId.toString())

        if (answer){
            
            const question = await this.questionsRepositoty.findById(answer.questionId.toString())
